...
const generateHtmlForMeals = (objectWithArrays) => {
  ...
  while (i < 7) {
    ...
    if (
      typeof objectWithArrays[0][i] === 'undefined' ||
      typeof objectWithArrays[1][i] === 'undefined' ||
      typeof objectWithArrays[2][i] === 'undefined'
    ) {
      if (i === 0) {
        ...
        errorParagraph.innerText =
          "Something wen't wrong, this could be because You entered some specifications we can't create a diet plan from, or an error occured. Please consider trying again.";
        ...
        const errorParagraph = document.createElement('p');
        errorParagraph.innerText =
          'We could only create this meal plan so far based off Your specifications. Different specifications could get You and better result.';
        errorDiv.appendChild(errorParagraph);
